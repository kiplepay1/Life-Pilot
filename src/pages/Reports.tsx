...
import { BarChart3, TrendingUp, TrendingDown, ShieldCheck, AlertTriangle, Car } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-slate-900">
          <Car className="h-4 w-4 text-brand-600" /> Monthly Grab Report
        </h2>
        {grabAgg.count === 0 ? (
          <p className="text-sm text-slate-500">No Grab sessions logged this month yet.</p>
        ) : (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div>
              <p className="text-xs text-slate-400">Sessions</p>
              <p className="font-semibold text-slate-800">{grabAgg.count}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400">Gross</p>
              <p className="font-semibold text-slate-800">{formatCurrency(grabAgg.totalGross, currency)}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400">Net</p>
              <p className="font-semibold text-emerald-700">{formatCurrency(grabAgg.totalNet, currency)}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400">Avg RM/hour</p>
              <p className="font-semibold text-brand-700">{formatCurrency(grabAgg.avgRmPerHour, currency)}</p>
            </div>
          </div>
        )}
      </div>
